...
const NewOrder = () => {
  ...
  const submit = (e) => {
    e.preventDefault()
    alert(`Order created (mock): ${form.customer} -> ${form.product} x${form.quantity}`)
  }

  return (
    <div className="w-full px-8 py-6">
      <h2 className="text-3xl font-bold text-blue-900 mb-6">New Order</h2>
      <form onSubmit={submit} className="bg-white rounded-lg shadow p-6 max-w-xl space-y-4">
        ...
        <button className="bg-blue-800 hover:bg-blue-900 text-white px-6 py-2 rounded transition">Create</button>
      </form>
    </div>
  )
}
